...
We also include the `spring-boot-maven-plugin` Maven plugin so that we can build a single, runnable "�ber-jar", which is convenient to execute and transport the written code.
